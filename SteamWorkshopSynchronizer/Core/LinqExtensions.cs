...
namespace SteamWorkshopSynchronizer.Core
{
    public static class LinqExtensions
    {
            ...
            Func<TLeft, TRight, TResult> resultSelector)
        {
            ...
            return rightItems.Where(r => !hashLK.Contains(rightKeySelector(r)))
                .Select(r => resultSelector(default(TLeft), r));
        }
        ...
        private static Expression<Func<TP, TC, TResult>> CastSMBody<TP, TC, TResult>(LambdaExpression ex, TP unusedP,
            TC unusedC, TResult unusedRes) => (Expression<Func<TP, TC, TResult>>)ex;
            ...
            Expression<Func<TLeft, TRight, TResult>> resultSelector)
        {
            ...
            var newleftrs =
                CastSMBody(Expression.Lambda(Expression.Invoke(resultSelector, argLeft, parmC), parmP, parmC),
                    sampleAnonLR, default(TRight), default(TResult));

            return leftItems.AsQueryable()
                .GroupJoin(rightItems, leftKeySelector, rightKeySelector, (left, rightg) => new { left, rightg })
            ...
            Expression<Func<TLeft, TRight, TResult>> resultSelector)
        {
            ...
            var newrightrs =
                CastSMBody(Expression.Lambda(Expression.Invoke(resultSelector, parmC, argRight), parmP, parmC),
                    sampleAnonLR, default(TLeft), default(TResult));

            return rightItems
                .GroupJoin(leftItems, rightKeySelector, leftKeySelector, (right, leftg) => new { leftg, right })
        ...
        private static Expression<Func<TP, TResult>> CastSBody<TP, TResult>(LambdaExpression ex, TP unusedP,
            TResult unusedRes) => (Expression<Func<TP, TResult>>)ex;
            ...
            Expression<Func<TLeft, TRight, TResult>> resultSelector)
        {
            ...
            var newrightrs = CastSBody(Expression.Lambda(Expression.Invoke(resultSelector, argLeft, argRight), parmLgR),
                sampleAnonLgR, default(TResult));

            return rightItems
                .GroupJoin(leftItems, rightKeySelector, leftKeySelector, (right, leftg) => new { leftg, right })
                .Where(lgr => !lgr.leftg.Any()).Select(newrightrs);
        }
